...
export async function GET() {
  try {
    ...
    const groupsWithParsedKeywords = groups.map((group) => ({
      ...group,
      keywords: JSON.parse(
        typeof group.keywords === 'string' ? group.keywords : JSON.stringify(group.keywords) || '[]'
      ),
    }))
  ...
  try {
    ...
    return NextResponse.json({
      ...
      group: {
        ...newGroup,
        keywords: JSON.parse(
          typeof newGroup.keywords === 'string'
            ? newGroup.keywords
            : JSON.stringify(newGroup.keywords)
        ),
      },
    })
  } catch (error: any) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      group: {
        ...updatedGroup,
        keywords: JSON.parse(
          typeof updatedGroup.keywords === 'string'
            ? updatedGroup.keywords
            : JSON.stringify(updatedGroup.keywords)
        ),
      },
    })
  } catch (error: any) {
