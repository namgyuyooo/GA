...
export async function POST(request: Request) {
    try {
        ...
        if (!Array.isArray(body)) {
            return NextResponse.json({ error: 'Request body must be an array of campaigns' }, { status: 400 })
        }
        ...
        for (let i = 0; i < body.length; i++) {
            ...
            if (!name || !baseUrl || !source || !medium || !campaign) {
                errors.push({ row: index + 1, error: 'Missing required fields (name, baseUrl, source, medium, campaign)' });
                continue;
            }
            ...
            try {
                ...
                if (e.code === 'P2002') { // Unique constraint violation
                    errors.push({ row: index + 1, error: `Campaign with source '${source}', medium '${medium}', and campaign '${campaign}' already exists.` });
                } else {
