...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cohortData = await prisma.keywordCohortData.findMany({
      ...
      orderBy: [{ cohortDate: 'desc' }, { impressions: 'desc' }],
    })
    ...
    if (cohortData.length === 0) {
      return NextResponse.json({
        ...
        isDemo: false,
        message:
          '⚠️ DB에 데이터가 없습니다. "데이터 업데이트" 버튼을 클릭하여 실제 데이터를 가져오세요.',
      })
    }
...
async function fetchKeywordDataFromGSC(propertyId: string) {
  ...
  if (!serviceAccountPath) {
    throw new Error(
      'Service account file not found in any of these paths: ' + possiblePaths.join(', ')
    )
  }
  ...
  if (!tokenData.access_token) {
    ...
    throw new Error(
      'Failed to get access token for GSC: ' +
  ...
  const gscResponse = await fetch(
    `https://searchconsole.googleapis.com/webmasters/v3/sites/sc-domain:rtm.ai/searchAnalytics/query`,
    {
      method: 'POST',
      headers: {
