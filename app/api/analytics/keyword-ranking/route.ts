...
function generateKeywordRankingData({ period, compareWith, limit }: any) {
  ...
  const opportunityKeywords = rankingChanges
  ...
  const cumulativeMetrics = {
    ...
    avgPosition:
      rankingChanges.reduce((sum, item) => sum + item.position, 0) / rankingChanges.length,
...
function analyzeCategoryPerformance(rankings: any[]) {
  ...
  return Array.from(categoryStats.values())
    .map((stats) => ({
      ...
      avgPosition:
        stats.keywords.reduce((sum: number, k: any) => sum + k.position, 0) / stats.keywords.length,
      avgCTR:
        stats.keywords.reduce((sum: number, k: any) => sum + k.ctr, 0) / stats.keywords.length,
