...
function analyzeContentTypePerformance(pages: any[]) {
  ...
  return Array.from(typeStats.values()).map(stats => ({
    ...
    avgPosition: stats.pages.reduce((sum: number, p: any) => sum + p.position, 0) / stats.pages.length,
    conversionRate: stats.totalConversions / stats.totalSessions,
    avgBounceRate: stats.pages.reduce((sum: number, p: any) => sum + p.bounceRate, 0) / stats.pages.length,
    avgSessionDuration: stats.pages.reduce((sum: number, p: any) => sum + p.avgSessionDuration, 0) / stats.pages.length,
