...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!accessToken) {
      return NextResponse.json({ error: 'Failed to initialize GA4 client' }, { status: 500 })
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch user journey data' }, { status: 500 })
  }
}
    ...
    const response = await fetch(
      `https://analyticsdata.googleapis.com/v1beta/properties/${propertyId}:runReport`,
      {
        ...
        body: JSON.stringify({
          ...
          dimensions: [{ name: 'pagePath' }, { name: 'nextPagePath' }],
    ...
    const response = await fetch(
      `https://analyticsdata.googleapis.com/v1beta/properties/${propertyId}:runReport`,
      {
        ...
        body: JSON.stringify({
          dateRanges: [{ startDate, endDate: 'today' }],
          dimensions: [{ name: 'pagePath' }],
          metrics: [{ name: 'sessions' }, { name: 'totalUsers' }, { name: 'returningUsers' }],
          limit: 50,
        }),
      }
    )
    ...
    const exitResponse = await fetch(
      `https://analyticsdata.googleapis.com/v1beta/properties/${propertyId}:runReport`,
      {
        ...
        body: JSON.stringify({
          dateRanges: [{ startDate, endDate: 'today' }],
          dimensions: [{ name: 'pagePath' }],
          metrics: [{ name: 'sessions' }, { name: 'exits' }, { name: 'exitRate' }],
          limit: 50,
        }),
      }
    )
