...
export default function PropertiesPage() {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(`✅ 기본 연결 성공: ${result.accounts?.length || 0}개 계정, ${result.properties?.length || 0}개 속성 발견`);
      } else {
            ...
            {propertyResults.length > 0 && (
              <div className="space-y-4">
                ...
                {propertyResults.map((result, index) => (
                  <div key={index} className={`p-4 border rounded-lg ${result.status === 'accessible' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                    <div className="flex justify-between items-center">
                      <span className="font-medium">속성 ID: {result.propertyId}</span>
                      <span className={`px-2 py-1 rounded text-xs ${result.status === 'accessible' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {result.status}
                      </span>
                    </div>
                    {result.sessions && (
