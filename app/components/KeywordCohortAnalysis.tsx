...
export default function KeywordCohortAnalysis({
  propertyId = '464147982',
}: KeywordCohortAnalysisProps) {
  ...
  const assignKeywordsToGroups = useCallback(() => {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          setKeywordGroups((groups) =>
            groups.map((group) =>
              group.id === groupId ? { ...group, keywords: updatedKeywords } : group
            )
          )
  ...
  const handleUpdateData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/analytics/keyword-cohort`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        <>
            ...
            <div className="flex items-center space-x-4">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  ...
                  {lastUpdate && (
                    <span className="text-gray-400 ml-1">(매일 9시 자동 업데이트)</span>
                  )}
                </p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {Object.entries(getGroupSummary())
                .filter(([key, summary]) =>
                  key === 'ungrouped'
                    ? (selectedGroup === 'ungrouped' && summary.keywordCount > 0) ||
                      (selectedGroup !== 'ungrouped' && summary.keywordCount > 0)
                    : summary.keywordCount > 0
                )
                .map(([groupId, summary]: [string, any]) => (
                  <div
                    key={groupId}
                  ...
                  >
                    ...
                    <div className="space-y-1 text-sm mt-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-gray-900">
                          {(summary.avgRetention * 100).toFixed(1)}%
                        </span>
                      </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200">
                          <details className="group">
                            <summary className="cursor-pointer text-xs text-gray-600 hover:text-gray-800 flex items-center justify-between">
                              <span>키워드 목록</span>
                              <span className="group-open:rotate-90 transform transition-transform">
                                ▶
                              </span>
                            </summary>
                            <div className="mt-2 space-y-1">
                              ...
                              <div className="mt-2 flex items-center space-x-1">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setSearchTerms({ ...searchTerms, [groupId]: e.target.value })
                                  }
          ...
          <div className="mt-8 w-full overflow-x-auto px-4">
                ...
                {getFilteredCohortData()
                  ...
                  .map((item) => (
                    <tr key={item.keyword}>
                      ...
                      <td className="whitespace-nowrap px-3 py-2 text-sm text-gray-900">
                        {item.initialUsers.toLocaleString()}
                      </td>
                      <td className="whitespace-nowrap px-3 py-2 text-sm text-gray-900">
                        <div className="w-full bg-gray-200 rounded-full h-3">
                          <div
                            className={
                              getRetentionBarColor(item.retentionWeek1 / (item.initialUsers || 1)) +
                              ' h-3 rounded-full'
                            }
                            style={{
                              width: `${Math.min(100, (item.retentionWeek1 / (item.initialUsers || 1)) * 100)}%`,
                            }}
                          ></div>
                        </div>
                      ...
                      <td className="whitespace-nowrap px-3 py-2 text-sm text-gray-900">
                        <div className="w-full bg-gray-200 rounded-full h-3">
                          <div
                            className={
                              getRetentionBarColor(item.retentionWeek4 / (item.initialUsers || 1)) +
                              ' h-3 rounded-full'
                            }
                            style={{
                              width: `${Math.min(100, (item.retentionWeek4 / (item.initialUsers || 1)) * 100)}%`,
                            }}
                          ></div>
                        </div>
                        <div className="text-xs text-center text-gray-500 mt-1">
                          {item.initialUsers > 0
                            ? ((item.retentionWeek4 / item.initialUsers) * 100).toFixed(1)
                            : '0.0'}
                          %
                        </div>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-3 py-2 text-sm">
                        {item.group &&
                        keywordGroups.find((g) => g.id === item.group) &&
                        item.group !== 'ungrouped' ? (
                          <span
                            className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${keywordGroups.find((g) => g.id === item.group)?.color}`}
                          >
            ...
            <div className="flex justify-end items-center mt-4 space-x-2">
              ...
              <span className="text-sm">
                {currentPage} / {Math.ceil(getFilteredCohortData().length / rowsPerPage)}
              </span>
              <button
                onClick={() =>
                  setCurrentPage((p) =>
                    Math.min(Math.ceil(getFilteredCohortData().length / rowsPerPage), p + 1)
                  )
                }
                disabled={currentPage === Math.ceil(getFilteredCohortData().length / rowsPerPage)}
                className="px-2 py-1 text-sm border rounded disabled:opacity-50"
              >
                다음
              </button>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewGroupData({ ...newGroupData, keywords: e.target.value })
                        }
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        rows={3}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setNewGroupData({ ...newGroupData, color: e.target.value })
                        }
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      >
