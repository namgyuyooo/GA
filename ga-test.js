...
async function testGA4Connection() {
  ...
  try {
    ...
    const cleanedServiceAccountKey =
      serviceAccountKeyRaw.startsWith("'") && serviceAccountKeyRaw.endsWith("'")
    ...
    if (error instanceof SyntaxError) {
      console.error(
        "GOOGLE_SERVICE_ACCOUNT_KEY의 JSON 형식이 잘못되었습니다. 키 값 전체를 작은따옴표(')로 감쌌는지 확인하세요."
      )
    }
